...
| D-11 | Batch mode checkbox on Generate: auto-runs Visualizer (PNG/file), Classifier, Theme Identifier, Summarizer (where file counts allow) and downloads all outputs | **Kevin's request, 2026-07-09** |
| D-12 | Text-level metrics embedded in `_cra.md` (schema 1.1): AFINN-165 sentiment, Gaucher et al. (2011) gendered-language index, abstractness (suffix HEURISTIC — Brysbaert et al. 2014 norms too large to bundle), agreeableness (marker HEURISTIC — no validated public lexicon), MTLD + TTR, lexical density, Flesch/FK readability, Coh-Metrix-style noun-overlap cohesion. Heuristics labeled in output | **Kevin's request, 2026-07-09** |
| D-12b | Six additional metrics (v1.2.0): Hyland (2005) hedges/boosters certainty index; pronoun person rates (pre-drop); word length + long-word share; passive-voice rate (HEURISTIC be+participle); Heylighen & Dewaele (1999) formality F-score; CRA network fragmentation (component count, largest-component share) in the network summary | **Kevin, 2026-07-09** |
| D-13 | Classifier: interactive hierarchical agglomerative clustering restored from original Crawdad — average linkage (UPGMA) on 1 − WR′, SVG dendrogram with draggable cut bar, live K/sizes/silhouette/singleton readout, cut captured in the downloaded report (schema 1.1). Merge heights validated against hand calculation | **Kevin's request, 2026-07-09** |
